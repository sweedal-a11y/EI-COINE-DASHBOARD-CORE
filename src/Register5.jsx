...
const Register5 = () => {
  ...
  // ── Mount / Unmount ───────────────────────────────────────────────────────
  useEffect(() => {
    logActivity('page_load', { page: 'register5_trading_amount' });
    ...
    console.log('%c💰 Trading Amount Step Loaded', 'color:#FFC300;font-size:16px;font-weight:bold;');
    ...
    return () => {
      ...
      console.log('===== TRADING AMOUNT STEP SESSION SUMMARY =====');
  ...
  const handleRadioChange = (e) => {
    ...
    setFormData((prev) => ({ ...prev, interestedInTokens: value }));
    logActivity('radio_change', { field: 'interestedInTokens', value });
    if (errors.interestedInTokens) {
      setErrors((prev) => ({ ...prev, interestedInTokens: '' }));
    }
  };
  ...
  const handleSelectChange = (e) => {
    ...
    logActivity('select_change', { field: 'estimatedAmount', value });
    if (errors.estimatedAmount) {
      setErrors((prev) => ({ ...prev, estimatedAmount: '' }));
    }
  };
  ...
  const validateField = (name, value) => {
    let error = '';
    if (name === 'interestedInTokens' && !value) {
      error = 'Please select an option';
    }
    if (name === 'estimatedAmount' && !value) {
  ...
  const validateAll = () => {
    const tokensOk = validateField('interestedInTokens', formData.interestedInTokens);
    const amountOk = validateField('estimatedAmount', formData.estimatedAmount);
    return tokensOk && amountOk;
  };
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      console.log('Trading amount step completed:', formData);
      setIsLoading(false);
      logActivity('form_success');
      // Navigate to Register7 page
      navigate('/register-7');
    }, 1200);
  };
  ...
  return (
    <div className="r5-container">
      {/* Logo */}
      <div className="r5-logo" onClick={() => logActivity('logo_clicked')}>
        <svg width="173" height="40" viewBox="0 0 173 40" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g clipPath="url(#clip0_r5_logo)">
            ...
            <mask id="mask0_r5_logo" style={{maskType:'luminance'}} maskUnits="userSpaceOnUse" x="3" y="9" width="39" height="22">
              <path d="M41.9299 9.61523H3.37549V30.3845H41.9299V9.61523Z" fill="white"/>
            </mask>
            <g mask="url(#mask0_r5_logo)">
      ...
      >
        <h1 className="r5-card-title">Create an Account</h1>

        {/* Progress Step */}
        <div className="r5-progress-container">
          <div className="r5-step-indicator">
            <div className="r5-step-icon">
              {/* Exchange / Trading icon */}
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M5.825 9.16626L2.5 12.4996L5.825 15.8329V13.3329H11.6667V11.6663H5.825V9.16626ZM17.5 7.49959L14.175 4.16626V6.66626H8.33333V8.33293H14.175V10.8329L17.5 7.49959Z" fill="white"/>
</svg>


            </div>
            <span className="r5-step-text">Trading Amount</span>
          </div>
          <div className="r5-progress-bar-wrapper">
            <div className="r5-progress-bar-track">
              <div className="r5-progress-bar-fill" style={{ width: '93%' }}>
                <div className="r5-progress-indicator">
                  <div className="r5-progress-indicator-ring"></div>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Form */}
        <form onSubmit={handleSubmit} className="r5-form" noValidate>

          {/* Are you interested in emireq tokens? */}
          <div className="r5-form-group">
            <label className="r5-form-label">Are you interested in emireq tokens?</label>
            <div className="r5-radio-group">
              <label 
                className={`r5-radio-option ${formData.interestedInTokens === 'Yes' ? 'selected' : ''}`}
                onMouseEnter={() => logActivity('radio_hover', { option: 'Yes' })}
              >
                <input
                  type="radio"
                  name="interestedInTokens"
                  value="Yes"
                  checked={formData.interestedInTokens === 'Yes'}
                  onChange={handleRadioChange}
                  onFocus={() => logActivity('element_focus', { element: 'radio_yes' })}
                />
                <span className="r5-radio-custom"></span>
                <span className="r5-radio-label">Yes</span>
              </label>
              <label 
                className={`r5-radio-option ${formData.interestedInTokens === 'No' ? 'selected' : ''}`}
                onMouseEnter={() => logActivity('radio_hover', { option: 'No' })}
              >
                <input
                  type="radio"
                  name="interestedInTokens"
                  value="No"
                  checked={formData.interestedInTokens === 'No'}
                  onChange={handleRadioChange}
                  onFocus={() => logActivity('element_focus', { element: 'radio_no' })}
                />
                <span className="r5-radio-custom"></span>
                <span className="r5-radio-label">No</span>
              </label>
            </div>
            <span className={`r5-error-msg${errors.interestedInTokens ? ' show' : ''}`}>{errors.interestedInTokens}</span>
          </div>

          {/* Estimated amount per transaction */}
          <div className="r5-form-group">
            <label htmlFor="estimatedAmount" className="r5-form-label">Estimated amount per transaction*</label>
            <div className="r5-select-wrapper">
              <select
                id="estimatedAmount"
                name="estimatedAmount"
                className={`r5-select${!formData.estimatedAmount ? ' placeholder-selected' : ''}${errors.estimatedAmount ? ' error' : ''}`}
                value={formData.estimatedAmount}
                onChange={handleSelectChange}
                onFocus={() => logActivity('element_focus', { element: 'estimatedAmount' })}
                onBlur={() => {
                  validateField('estimatedAmount', formData.estimatedAmount);
                  logActivity('element_blur', { element: 'estimatedAmount' });
                }}
              >
                <option value="" disabled hidden>Select an approximate amount</option>
                {TRANSACTION_AMOUNTS.map((amt) => (
                  <option key={amt} value={amt}>{amt}</option>
                ))}
              </select>
              <span className="r5-select-chevron">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M4 6L8 10L12 6" stroke="#717182" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </span>
            </div>
            <span className={`r5-error-msg${errors.estimatedAmount ? ' show' : ''}`}>{errors.estimatedAmount}</span>
          </div>

          {/* Next Button */}
          <button
            type="submit"
            className={`r5-btn-next${isLoading ? ' loading' : ''}`}
            disabled={isLoading}
          ...
          {/* Login Link */}
          <div className="r5-login-text">
            Already have an account?{' '}
            <a href="#" className="r5-login-link" onClick={handleLoginClick}>
              Log in
            </a>
          </div>
        </form>
